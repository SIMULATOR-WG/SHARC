...
class TopologyIndoor(Topology):
    ...
    def plot(self, axis: matplotlib.axes.Axes, top_view=True):
        if top_view:
            self.plot_top_view(axis)
        else:
            self.plot_side_view(axis)

    def plot_top_view(self, axis: matplotlib.axes.Axes):
        # create the building
        ...
        axis.scatter(self.x, self.y, color='k', edgecolor="k", linewidth=2, label="Base station")

    def plot_side_view(self, axis: matplotlib.axes.Axes):
        ...
        axis.scatter(self.x, self.height - 0.05, color='k', edgecolor="k", linewidth=2, label="Base station")
...
if __name__ == '__main__':
    ...
    axes = plt.gca()
    # axes.set_xlim([-param.street_width, param.n_colums*3*param.intersite_distance + (param.n_colums-1) \
    # *param.street_width + param.street_width])

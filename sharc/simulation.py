...
class Simulation(ABC, Observable):
    ...
    def __init__(self, parameters: Parameters, parameter_file: str):
        ...
        self.wrap_around_enabled = self.parameters.imt.wrap_around and \
                                   (self.parameters.imt.topology == 'MACROCELL' or
                                    self.parameters.imt.topology == 'HOTSPOT') and \
        ...
        if (self.overlapping_bandwidth == self.param_system.bandwidth and not self.parameters.imt.interfered_with) \
            or (self.overlapping_bandwidth == self.parameters.imt.bandwidth and self.parameters.imt.interfered_with):
    ...
    def plot_scenario(self):
        ...
        ax.scatter(self.ue.x, self.ue.y, color='b', edgecolor="w", linewidth=0.5, label="UE")

        # wedge = Wedge((0, 0), 300, 0, 360, 290, color='b', alpha=0.2, fill=True)
        # ax.add_artist(wedge)

        # Plot UE's azimuth
        d = 0.05 * self.topology.cell_radius

...
class SimulationDownlink(Simulation):
    ...
    def calculate_sinr_ext(self):
        ...
        self.ue.ext_interference[ue] = (eirp + 30) - gw_antenna_factor[0,ue] - self.path_loss_imt_system[0,ue] \
        ...
        self.ue.pfd = protection_criteria + 10*np.log10(4*np.pi/(lambda_imt**2)) \
                            + self.parameters.imt.ue_ohmic_loss \
        ...
        for bs in bs_active:
            ue = self.link[bs]
            self.ue.pfd_level[ue[0]] = 10*np.log10(np.sum(10**(0.1*pfd_level[slot_1,ue[0]])))
            self.ue.pfd_level[ue[1]] = self.ue.pfd_level[ue[0]]
            self.ue.pfd_level[ue[2]] = 10*np.log10(np.sum(10**(0.1*pfd_level[slot_2,ue[2]])))

        #self.ue.pfd_interfered[ue] = self.ue.pfd < self.ue.pfd_level[ue]
    ...
    def collect_results(self, write_to_file: bool, snapshot_number: int):
        ...
        for bs in bs_active:
            ...
            if self.parameters.imt.interfered_with:
                ...
                self.results.imt_dl_pfd.extend(self.ue.pfd.flatten().tolist())

...
class SimulationDownlinkTest(unittest.TestCase):
    ...
    def test_simulation_2bs_4ue_fss_ss(self):
        ...
        npt.assert_allclose(self.simulation.ue.rx_interference, 
                            np.array([p_tx-(97.55-2-10)-4-3,  p_tx-(94.72-2-11)-4-3, p_tx-(93.27-1-22)-4-3, p_tx-(97.05-1-23)-4-3]),
                            atol=1e-2)
        ...
        npt.assert_allclose(self.simulation.ue.total_interference, 
                            10*np.log10(np.power(10, 0.1*np.array([p_tx-(97.55-2-10)-4-3,  p_tx-(94.72-2-11)-4-3, p_tx-(93.27-1-22)-4-3, p_tx-(97.05-1-23)-4-3])) +
                                        np.power(10, 0.1*(-88.44))),
                            atol=1e-2)
        # check SNR 
        npt.assert_allclose(self.simulation.ue.snr, 
                            np.array([-67.48 - (-88.44),  -77.36 - (-88.44), -67.54 - (-88.44),  -57.00 - (-88.44)]),
                            atol=1e-2)        
        # check SINR
        npt.assert_allclose(self.simulation.ue.sinr, 
                            np.array([-67.48 - (-83.76), -77.36 - (-80.89), -67.54 - (-70.22), -57.00 - (-72.94)]),
                            atol=1e-2)        
...
=======
        interference = 10 - 10*np.log10(2) - np.array([203.52-51-1, 203.52-51-1, 203.52-51-2, 203.52-51-2]) + 10*math.log10(45/100)
>>>>>>> origin/development
...
>>>>>>> origin/development
    ...
    def test_simulation_2bs_4ue_fss_es(self):
        ...
        interference = 10 - 10*np.log10(2) - np.array([118.47-50-1,  118.47-50-1,  119.29-50-2,  119.29-50-2]) + 10*math.log10(45/100)
        rx_interference = 10*math.log10(np.sum(np.power(10, 0.1*interference)))
        self.assertAlmostEqual(self.simulation.system.rx_interference,
                               rx_interference,
                               delta=.01)
